  ...
  <div style="margin-top: 60px;margin-left:80px;border: 0 solid red;">
    <el-form style="margin-left: -40px" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px"
      ...
      <el-form-item label="搜索范围:" prop="key">
        <el-select v-model="ruleForm.key" style="width: 160px;float: left" placeholder="请选择搜索范围">
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          axios.get('http://localhost:9090/dormitory/search', {params: _this.ruleForm}).then(function (resp) {
    ...
    page(currentPage) {
      ...
      if (_this.ruleForm.value === '') {
        axios.get('http://localhost:9090/dormitory/list/' + currentPage + '/' + _this.pageSize).then(function (resp) {
          _this.tableData = resp.data.data.data
          _this.total = resp.data.data.total
        })
      } else {
